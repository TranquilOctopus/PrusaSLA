...
#include <libslic3r/Format/SlaArchiveFormat.hpp>
...
TEST_CASE("SlaArchiveFormatRegistry returns SL1 format", "[SlaArchiveFormat]") {
    auto& registry = sla::SlaArchiveFormatRegistry::instance();
    
    // Register formats (idempotent)
    sla::register_sla_archive_formats();
    
    // Test SL1 format
    auto sl1 = registry.get("SL1");
    REQUIRE(sl1 != nullptr);
    REQUIRE(sl1->name() == "SL1");
    REQUIRE(sl1->file_data_type() == sla::FileDataType::sl1_png);
    ...
    REQUIRE(sl1_svg->file_data_type() == sla::FileDataType::sl1_svg);
