...
- [ ] **M2.3b** Close the support-tool parity gaps listed as missing in `parity/support-points.md`: multi-select, select all, Delete key, lock islands, cone-shaped point visuals, clipping reset and mouse-wheel clipping, live head-diameter update for selected points, persistent selection. · M · needs M2.3
...
- [ ] **M2.10** `[human]` Verify the Lychee support option list in [supports/lychee-features.md](supports/lychee-features.md): a draft covering all option groups, including stem geometry. Correct names and typical values, add missing options, and tick the Verified column. From your knowledge only; Lychee files are not decoded. · S · needs —
- [ ] **M2.11** Parity table `doc/sla-fork/supports/lychee-parity.md`: map every M2.10 option onto existing SLA keys and mark it covered, partial or missing. Existing keys: `support_head_front_diameter`, `support_head_penetration`, `support_head_width`, `support_pillar_diameter`, `support_small_pillar_diameter_percent`, `support_base_diameter`, `support_base_height`, `support_pillar_connection_mode`, `support_max_bridge_length`, `support_critical_angle`, `support_object_elevation`, `support_tree_type`, and the `pad_*` family. Split every gap into its own todo below. · S · needs M2.10
