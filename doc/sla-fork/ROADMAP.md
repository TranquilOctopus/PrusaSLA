...
- [ ] **M1.7** SLA-first app setting, on by default: the first run defaults to SLA and FFF-only UI is hidden. · M · needs M0.8, M1.6
  State: attempted and rejected on branch `sla/M1.7-sla-first` (not merged): it declared a method twice, reached private members of `PrinterScreen` from outside, and changed `ProjectInteractor::new_project_with_modification` signatures. Redo from scratch against the real APIs.
