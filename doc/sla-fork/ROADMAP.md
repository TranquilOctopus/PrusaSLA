...
- [x] **M1.7c** Hide the FFF-only sections of `LogicalPrinterSettingsDialog` for SLA printers: the Sheet heading and combo, the Nozzles heading, the nozzle list and its "Invalid combination" warning. Follow `update_color_mix_visibility()`, which already gates the colour-mix button on FFF. Evidence: found 2026-09-22 while fixing the Preview crash; SLA has no sheets or nozzles, so both sections are empty. · S · needs M1.7
  Result: the Sheet heading and picker, the Nozzles heading, the nozzle list and its warning are hidden for SLA printers by update_fff_section_visibility(), which follows update_color_mix_visibility(). Review added the call sites the job missed (update_settings_data, and after update_warning in on_about_to_show) and kept the sections visible when no printer is selected. Builds and the suites pass; not yet looked at in the running app.
- [ ] **M1.8** SLA path in the welcome dialog, plus SLA hints and notifications. Also fix the “Export gcode to a file” tooltip for SLA (evidence: `ux/journeys.md` section H). · M → split · needs M1.7, M1.3
  - [x] **M1.8a** Welcome and export wording: replace the welcome dialog's "what's new" screen, which still shows upstream PrusaSlicer 3.0 release notes ("runs safely alongside 2.x", "your 2.x projects load"), with text about this fork; and make the Preview export tooltip say what SLA exports instead of "Export gcode to a file". · S · needs M1.7
    Result: the welcome dialog's what's-new screen describes this fork instead of upstream's PrusaSlicer 3.0 release notes (text supplied verbatim, all translatable), and the Preview export tooltip reads "Export print file" for SLA. Builds and the suites pass; not yet looked at in the running app.
  - [ ] **M1.8b** SLA hints and notifications along the first-run and plater flow. · M · needs M1.8a
...
- [ ] **M1.11** SLA sidebar summary (PLAN F5). · M → split · needs M1.10, M1.3
  - [x] **M1.11a** Print summary figures for the selected bed, per wireframe W4: resin ml, cost, bottles and layer count, an en dash where a value is not available, visible only for SLA. Lives in its own files and is embedded in the bed sidebar section, so the PlaterRenderModule hotspot is not touched. · M · needs M1.10, M1.3
    Result: `SidebarSlaSummary` shows resin ml, cost and bottles, and layer count for the selected bed in the bed sidebar section, with an en dash where a value is missing; hidden for FFF. Formatting helpers are unit-tested. Not yet checked by eye in the running app.
...
- [ ] **M1.13** User test feedback, 2026-09-23 (first hands-on session with the app). · L → split
  - [x] **M1.13a** Say "build plate", never "bed", wherever an SLA printer is active: sidebar sections, menus, notifications, tooltips, dialogs. Pick the string by printer technology so the FFF wording stays for upstream. · M · needs —
    Result: By OpenCode, reviewed. The SLA tools say "build plate" always; Place on bed, the Bed reference frame, Out of bed and the per-plate slicing warning switch by technology via `is_sla_active()` (`App/IsSlaActive.hpp`, safe before a printer is selected). Left as "bed": the arrange dialog, menu items, undo step names and two error texts, whose code has no project access (follow-up M1.13f). Review fixed a label read before its member was initialised.
  - [x] **M1.13b** Preset slots for SLA (decided 2026-09-23): **Printer · Resin · Supports & raft**. Rename the SLA "material" preset to "Resin" and the SLA "print" preset to "Supports & raft" in every visible place, and hide the FFF nozzle slot that sits next to the resin in the sidebar for SLA printers. Labels and visibility only; no settings move yet. · M · needs —
    Result: By OpenCode, reviewed. For SLA the sidebar print slot reads "Supports & raft", the settings tab likewise, the material tabs read "Resin", and the nozzle button next to each resin is hidden. Review fixed a null dereference (visibility set before the button existed) and a wrong assignment. Not yet seen in the running app.
  - [ ] **M1.13c** Move the exposure and layer settings into the resin preset, so a resin preset is one resin on one printer: layer height, normal and bottom exposure, bottom and transition layer counts, lift and wait timings, light-off delay. Supports, raft and hollowing stay in "Supports & raft". Needs a migration for existing presets and 3MF projects, and it touches the `ConfigDefsSLA.cpp` hotspot. · L → split · needs M1.13b
    Note 2026-09-23 (second test): the user found the current resin settings contradict each other; part of this move.
  - [ ] **M1.13d** Audit the SLA settings panels and hide every option that does nothing for SLA (hide, don't delete). List what was hidden and why in the result note. · M · needs —
    Progress: OpenCode wrote an inventory in `doc/sla-fork/sla-settings-audit-{a,b,c}.md` (keep/hide/unsure per visible setting). The verdicts are shallow and need review before anything is hidden: it marks `elefant_foot_compensation` FFF-only (SLA uses it) and keeps the SL1-only tilt timings, which do nothing on non-tilt printers such as the Photon Mono M5.
...
- [x] **M2.7b** Extend the 3MF SLA data so nothing is lost on save/reload: support point `type` (only an island flag is stored today, so slope and manual_add both come back as manual_add), `sla_points_status`, and per-object `object_settings_sla`, which is not written at all. Keep older files readable and keep the file readable by upstream PrusaSlicer. Evidence: the M2.7 round-trip tests in `3MFTests.cpp`. · M · needs M2.7
  Result: support point type, sla_points_status and object_settings_sla survive save and reload. A compatibility test unzips a saved 3MF, strips the three new keys and checks the defaults come back. The type was being lost by the reader, not the writer: the json parser stores every non-negative integer as unsigned, and get_ptr<number_integer_t> in the nlohmann version we ship only accepts signed, so any signed field of 0 or more read from a 3MF fell back to its default. Fixed in the shared value_from_json helper.
  State: partly done. 3MF writes and reads keys for point type and every per-point override; the float overrides round-trip correctly (tested). `type` still does not survive. Ruled out: a second serialiser, key collisions, the legacy support-points file, and the integer width (now written as `json::number_integer_t`). Next step: dump the written json for a slope point and see whether the key is present in the archive. `sla_points_status` and `object_settings_sla` are still not written.
...
- [x] **M2.12** Per-point support sizes (Lychee light/medium/heavy): per-point tip, stem and base dimensions stored on each support point; editor controls in the support tool; the support tree honours per-point values. It changes `Domain/SLA/SupportPoint.hpp` (heavy rebuild), so batch the header change with M2.13. · L → split · needs M2.2, M2.11
  Result: per-point stem diameter, base diameter and base height (0 = use global), honoured by the support tree, with dialog controls that apply to the selection and Light/Medium/Heavy presets (tip/stem/base 0.30/0.8/2.0, 0.45/1.2/3.0, 0.60/1.8/4.0 mm, named constants to tune against Lychee). Values survive undo and 3MF. Not checked in a running app.
- [x] **M2.13** Tip geometry: tip shape (cone, sphere contact), tip length and contact depth, both globally and per point, carried through to the support tree mesh. · L → split · needs M2.11
  Result: per-point `tip_length`, `contact_depth` and `tip_shape` (Default/Cone/Ball) on `SupportPoint`; the support tree uses tip length and contact depth per point. NOT done: tip_shape is stored but the head geometry is unchanged, so Cone and Ball behave like Default; no UI yet. Persisted in 3MF and undo.
- [x] **M2.14a** Raft options, settings layer: a `raft_type` choice (none, full plate, around object, skate) plus the shared knobs raft height, wall thickness, expansion/margin, slope and object gap, mapped onto the existing pad generator (`pad_enable`, `pad_wall_height`, `pad_wall_thickness`, `pad_brim_size`, `pad_wall_slope`, `pad_around_object`, `pad_object_gap`). The mapping lives in one tested pure function so a raft type is a named bundle of pad values, not scattered ifs. · M · needs M2.11
  Result: `raft_type` setting (None, Full plate, Around object, Skate) plus a pure mapping `Domain::SLA::RaftPreset` turning a type and the shared knobs (height, wall thickness, expansion, slope, object gap) into pad values; applied where the pad config is built in `SLAPrint.cpp` (two call sites, guarded so older configs without the key keep working). Tests in `tests/sla_print/sla_raft_preset_tests.cpp` (SLA suite 55 -> 62 cases). Skate is Around-object with half the brim and a 70 degree wall: named constants, explicitly a starting point to tune against Lychee and Chitubox. No UI yet (M2.14c); grid/honeycomb, tapered skate profile and interface layer still need generator work (M2.14b).
  Note: our pad is a bed-adhesion base with a cavity; Lychee's and Chitubox's rafts are shaped plates. Types that the current generator cannot express (grid/honeycomb infill, tapered skate profile, raft interface layer) belong to M2.14b, not here.
- [ ] **M2.14b** Raft shapes the pad generator cannot make today: grid or honeycomb infill, a tapered skate profile, and a separate raft interface layer. Needs work in the pad mesh generator (`src/libslic3r/src/libslic3r/SLA/Pad.*`). · L → split · needs M2.14a
- [ ] **M2.14c** Raft UI: the raft type dropdown and its knobs in the SLA print settings, with the preset bundles from M2.14a. · M · needs M2.14a, M1.3
  Review: the merged change only relabels the pad settings as raft settings. Its tooltips (and the tests that pin them) claim the raft type sets the raft height, wall thickness and gap, and switches "Use raft" on; it does neither. Fix under M2.14d before ticking.
- [ ] **M2.14d** Make `raft_type` the single source of truth. Engine: `generate_pad`, `slice_supports`, `SLAPrintObject::get_elevation` and the pad mesh accessors read `pad_enable` directly while `is_zero_elevation` and `make_pad_cfg` derive it from `raft_type`, so raft None with "Use raft" on (the default) disagrees with itself; derive it in one place. UI: hide the "Use raft" and "Raft around object" checkboxes that the raft type replaces (hide, don't delete, for upstream mergeability). Correct the M2.14c tooltips and `SlaRaftSettingsTests.cpp` to say what the raft type really sets (on/off, around the object, and Skate's halved expansion and 70° slope). · M · needs M2.14c
- [ ] **M2.15** Bracing and cross-bracing controls beyond `support_pillar_connection_mode`, if M2.11 marks them missing. · M · needs M2.11
- [x] **M2.16** Stem geometry: stem cross-section (round, square, polygon side count), stem taper, and knot/joint balls between tip and stem, both globally and per point, generated in the support tree mesh. Batch the `SupportPoint.hpp` change with M2.12/M2.13. · L → split · needs M2.11
  Result: per-point `stem_sides`, `stem_taper` and `knot_radius` stored on `SupportPoint` and persisted. NOT done: the pillar mesh builder only makes round, untapered pillars, so these values are read but not built, and there is no knot geometry or UI. Honouring them needs a mesh-builder change; the call site carries a comment.
- [ ] **M2.17** Supports are an explicit step before slicing, never done by the slicer. The workflow (decided 2026-09-22): rotate and arrange; support the models, by hand in the support tool or with **Auto support** on the selected models or all of them; then slice, preview and save. Upstream does the opposite: `SLAPrint::Steps::support_points` (SLAPrintSteps.cpp ~790) runs the generator on every slice and treats the model's points as seeds it fills in around. · L → split · needs M2.3
  - [x] **M2.17a** Engine: slicing uses each model's `sla_support_points` exactly as they are, with no generation at slice time. A model with no points is sliced without supports (decided: slice it and warn, do not refuse). `supports_enable` stays the master switch. · M · needs M2.3
    Result: M2.17a1 + M2.17a2, by OpenCode, reviewed. Slicing uses each model's own points; the generator runs only for the support tool's SliceUntilStep{slaposSupportPoints}. An object without points (`SLAPrintObject::has_supports()`) sits on the plate: no elevation, no raft under it (a raft around it still works), so NoPadGenerated no longer blocks export. Any change of points invalidates the support step, and a switch between none and some re-slices the object. Tests in `SlaUnsupportedSliceTests.cpp`.
    Note: slicing with supports off and the pad on (the default) used to SIGSEGV, because the pad and support-slicing steps read the support tree mesh, which is null when supports are off. Fixed in `SLAPrintSteps.cpp` during M5.4a and covered by `SlaUnsupportedSliceTests.cpp` (raft around the object, or none). Still open for this todo: with supports off and a raft *under* the object (raft Full, the default), slicing fails with `NoPadGenerated`, as upstream does, because that pad has nothing to hold. An unsupported model must get no raft or a raft around the object instead, or the whole bed fails to slice.
  - [ ] **M2.17b** **Auto support** action for the selected models or for all of them: run the generator (the support tool's existing slice-until-`slaposSupportPoints` path) and write the result into each model's `sla_support_points` with status `AutoGenerated`, so the points show in the support tool, can be edited, and are saved in the 3MF. When a model already has points, ask each time (decided): keep them and add around them, or replace them. · M · needs M2.17a
...
- [x] **M3.3** `ForeignResinProfile` struct, `IResinProfileReader` interface, and reader registry with `sniff()`. The `.cfg` extension is generic, so detect by content (e.g. a `normalExposureTime:` line), not just the extension. · S · needs M0.2
...
- [x] **M4.9** Per-layer stats: area, island count, peel-force estimate (PLAN B6). · M · needs M0.6
  Result: `SLA/LayerStats.{hpp,cpp}` computes exposed area per layer (mm2) and a peel-force estimate; both fill the `layer_areas` and `layer_peel_force` fields reserved in M0.6, alongside the island count already in `issues`. Tests in `tests/sla_print/sla_layer_stats_tests.cpp` (SLA suite 48 -> 55 cases). The peel force is area times a placeholder constant, documented as such: only relative values are meaningful until someone measures a real printer. Nothing displays these yet (M5.7).
...
- [ ] **M5.3.pw-b** Anycubic newer formats (`.pm5`, `.pm5s`, `.pm7`) — **`.pm5` first: it is the format the maintainer's Photon Mono M5 reads, so nothing exported from this fork prints on that machine until it exists**: per-variant tables (resolution, pixel size, format version, preview size) on top of the M5.3.pw-a writer. · L → split per format · needs M5.3.pw-a
  ...
  Update 2026-09-22: a Photon Workshop `.pm5` sliced for the M5 arrived (kept in `local-samples/`). It is an unencrypted Photon Workshop version-517 container whose layers use the same PW0 run-length encoding the Anycubic encoder already writes, verified to the pixel. Layout written up in `doc/sla-fork/formats/pm5.md`. That makes `.pm5` the cheap path for this printer, so the `.ctb` question above no longer blocks it. The sample also showed the presets had `display_orientation: portrait`, which would have transposed every layer; all six are now `landscape`.
  - [x] **M5.3.pw-b1** `.pm5` writer following `doc/sla-fork/formats/pm5.md`: the version-517 container (file mark and address table, HEADER, PREVIEW, colour table, LAYERDEF, EXTRA, MACHINE, software block, MODEL) around the existing PW0 layer encoder. Reproduce the declared section lengths as observed. Put our own identity in the software block. Check it with a script that parses our output and the local sample with the same reader and compares layout field by field. · M · needs M5.3.pw-a
    Result: `store_pm5` writes the version-517 container in the sample's physical block order around the PW0 layers; tests pin the magic, version, area count and the MACHINE, software and MODEL block offsets. Not yet compared field by field with the sample through `pm5_layout.py --compare`, and not yet printed.
...
- [ ] **M5.3.profiles-b** Community resin presets per layer height, so the first layer matches the rest. `initial_layer_height` is a material setting, and both generic resins hard-code 0.05, so the 0.03 mm print preset gets a 0.05 mm first layer (seen 2026-09-22 at the bottom of the layer slider). Restructure `preset-sla-material.yaml` into layer-height variants the way the Prusa bundle does (`variants:` with `condition: print.layer_height == …`), one per print preset. This changes preset ids, so check that a project or selection referring to the old ids still loads; a bundle mistake crashed startup earlier. · S · needs M5.3.profiles
...
- [x] **M6.8** A failed slice must not close the app. Today any exception on the slicing thread, including running out of memory, is re-raised on the main thread as `FatalSlicingError` (`BackgroundProcess.cpp:274`, `SlicingInteractor.cpp:234`) and terminates the process, losing unsaved work. Report it on the bed and keep the project open; keep genuinely unrecoverable cases fatal. · M · needs —
  Result: `BackgroundProcess` maps `std::bad_alloc` to `OutOfMemory` and any other `std::exception` to `InternalError`; both are reported on the bed with their own messages instead of terminating the app.
- [x] **M6.9** Saving an edited resin preset crashed the app (`PresetSaver.cpp:14`, `ASSERT(!exprs.empty())`): the community SLA resins have no `condition`, and the saver assumed every preset has one. · S · needs —
