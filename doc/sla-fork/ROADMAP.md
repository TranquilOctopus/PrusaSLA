...
- [ ] **M2.10** `[human]` Lychee support feature list: write down, from experience, every Lychee support option you use and its typical values. Include tip shape, tip length, contact diameter, contact depth, stem/pillar and bottom/base dimensions, light/medium/heavy presets, raft types, and bracing. Save it as `doc/sla-fork/supports/lychee-features.md`. From your knowledge only; Lychee files are not decoded. · S · needs —
