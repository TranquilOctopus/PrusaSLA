...
- [ ] **M2.14a** Raft options, settings layer: a `raft_type` choice (none, full plate, around object, skate) plus the shared knobs raft height, wall thickness, expansion/margin, slope and object gap, mapped onto the existing pad generator (`pad_enable`, `pad_wall_height`, `pad_wall_thickness`, `pad_brim_size`, `pad_wall_slope`, `pad_around_object`, `pad_object_gap`). The mapping lives in one tested pure function so a raft type is a named bundle of pad values, not scattered ifs. · M · needs M2.11
