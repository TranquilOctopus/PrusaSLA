...
void load_objects(
    ...
    for (const json &object_json : objects_json) {
        ...
        if (auto sla_points_status_json = object_json.find(SLA_POINTS_STATUS);
            ...
            // PointsStatus is NoPoints, Generating, AutoGenerated, UserModified
            // (SupportPoint.hpp:12). An out-of-range value from a newer or corrupt file leaves
            // the default rather than producing an invalid enum.
            if (status_int >= 0 &&
                status_int <= static_cast<json::number_integer_t>(Domain::SLA::PointsStatus::UserModified)) {
                for (ModelObject* mo_ptr : mos)
