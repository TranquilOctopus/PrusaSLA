...
TEST_CASE("ModelSerialize - roundtrip with SLA support points and drain holes", "[ModelSerialize][SLA][undo]")
{
    ...
    auto* model_object = new ModelObject();
    ...
    original_model.objects.push_back(model_object);
    ...
    REQUIRE(recovered_object->sla_support_points.size() == 3);
...
TEST_CASE("ModelSerialize - roundtrip with multiple objects", "[ModelSerialize][SLA][undo]")
{
    ...
    original_model.objects.push_back(obj1);
    original_model.objects.push_back(obj2);
    original_model.objects.push_back(obj3);
